...
説明:
    ...
    分類機を指定するか([-s]オプションで指定)、リファレンス配列および系統データ([-f],[-x]オプションで指定)を指定するかで、
    scikit-learnとblastのどちらを実行するかが判定されます。
...
使用例:
    # sklearn
    $CMDNAME -s silva-138-99-nb-classifier.qza repset.qza table.qza
    ...
    Q2ENV=qiime2-2022.2
    $CMDNAME -s silva-138-99-nb-classifier.qza -e \${CENV} -q \${Q2ENV} repset.qza table.qza
...
# 1-1. オプションの入力処理
while getopts s:f:x:e:q:n:m:o:b:O:h OPT
do
  case $OPT in
    "s" ) VALUE_s="$OPTARG";;
    "f" ) VALUE_f="$OPTARG";;
...
if [[ -n "${Q2DB}" && -n "${VALUE_s}" && -z "${VALUE_f}" && -z "${VALUE_x}" ]] ; then
    CLF=$(dirname $Q2DB)/$(basename $Q2DB)/${VALUE_s}
    echo -e "# ${CLF} was designated as the classifier." 
elif [[ -z "${Q2DB}" && -n "${VALUE_s}" && -z "${VALUE_f}" && -z "${VALUE_x}" ]] ; then
    CLF=${VALUE_s}
    echo -e "# ${CLF} was designated as the classifier."

elif [[ -n "${Q2DB}" && -z "${VALUE_s}" && -n "${VALUE_f}" && -n "${VALUE_x}" ]] ; then
...
elif [[ -z "${Q2DB}" && -z "${VALUE_s}" && -n "${VALUE_f}" && -n "${VALUE_x}" ]] ; then
    REFA="${VALUE_f}"
    RETAX="${VALUE_x}"
    echo -e "# ${REFA} ${RETAX} were designated as the classifier."
else
...
if [[ -z "$VALUE_m" ]]; then  : ;
elif [[ -n "$VALUE_m" && -f "$VALUE_m" ]]; then 
 META=${VALUE_m}; 
elif [[ -n "$VALUE_m" && ! -f "$VALUE_m" ]] ; then
    echo -e "[ERROR] The file of metadata ${VALUE_m} does not exist."
    exit 1
fi
